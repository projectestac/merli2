...
public class DataSourceSNJDBC  implements DataSource{
	...
	public void init(Hashtable propietats) {
		...
		try {
			...
			c=cb.getConnection();
			try {
				if (AccesBD.executeExist("cur_level","id_node","0","",c)==0){
					AccesBD.executeInsert("cur_level","id_node,v_term,v_category,id_nodecur_level","0,'origen','origen',0",c);	
				}
				if (AccesBD.executeExist("cur_content","id_node","0","",c)==0){
					AccesBD.executeInsert("cur_content","id_node,v_term,v_category","0,'origen','origen'",c);	
				}
		...
		public Hashtable getNode(int idNode, String nodeType) throws SemanticException {
			...
			try {
				c = cb.getConnection();
				Map m =  AccesBD.getObject("cur_"+nodeType,l,id_node+" = "+idNode,c);
				c.close();
				dto = (Hashtable)mapToNodesDTO(m, nodeType).get(0);				
			} catch (Exception e) {
				logger.error("The node "+idNode+nodeType+" doesn't exists.");
...
//			 TODO Auto-generated method stub
			...
			try {
				c = cb.getConnection();
				Map m =  AccesBD.getObjectList("cur_notes",l,"id_node = "+idNode+" AND v_type='"+nodeType+"'"," d_contribution DESC ",c);
				c.close();
				...
				logger.error("The note "+idNode+nodeType+" doesn't exists.");
				SemanticException the = new SemanticException("NonExistentNode");
				the.setCode(SemanticException.NONEXISTENTNODE);
				the.setStackTrace(e.getStackTrace());
			}finally{
		...
		public List getNodes(List lnod, String nodeType) throws SemanticException {
			...
			try {
				cb = this.connectBD();
				c=cb.getConnection();
				m =  AccesBD.getObjectList("cur_"+nodeType,l,condicio,order,c);
				c.close();
				lDTO = this.mapToNodesDTO(m, nodeType);
			} catch (Exception e) {
				logger.error("Error loading list of nodes "+nodeType+".");
				SemanticException the = new SemanticException("NonExistentNode");
				the.setCode(SemanticException.NONEXISTENTNODE);
				the.setStackTrace(e.getStackTrace());
			}finally{
		...
		public List getRelations(int idNode, String nodeType , String type, int direction) throws SemanticException {
			...
			try {
				cb= this.connectBD();
				c = cb.getConnection();
				Map m = AccesBD.getObjectList(table,l,"("+condi+")","", c);
				c.close();
				List l1, l2, l3 = null;
				...
				logger.error("Error loading relations of:"+idNode+nodeType+":"+e.getMessage());
			}catch (NullPointerException e) {
				logger.error("Error on connection:"+idNode+nodeType+":"+e.getMessage());
			}finally{
				this.disconnectBD(cb);
			}
		...
		private void disconnectBD(ConnectionBean cb){
			...
			broker.freeConnectionBean(cb);
				...
				logger.error("Error disconnecting connection."+e.getMessage());}//missError ="error.desconnexio.bd";}
		}
		...
		public int addNewNode(Hashtable DTO) throws SemanticException {
			...
			try{
				...
				c = cb.getConnection();
				idNode = AccesBD.getNext("cur_nodes", c);
				c.close();
			...
			try {
				...
				c = cb.getConnection();
				res = AccesBD.executeInsert("cur_"+sNType,camps,insert,c);
				c.close();
				...
				if (DTO.containsKey("note")){
					...
					try {
						if (DTO.get("note") != null && ((String)DTO.get("note")).length() > 2){
							...
							c = cb.getConnection();
							int idNote = AccesBD.getNext("seq_notes", c);
							...
							notenode += "SYSDATE";							
							res = AccesBD.executeInsert("cur_notes",notenode,c);
							c.close();
						}
					} catch (Exception e) {
...
//				Inserta les noves relacions amb thesaurus keys.
			/*	if (((Hashtable) DTO.get("properties")).containsKey("thesaurusKeys")){
				...
				// TODO Auto-generated catch block
				logger.error("Error while adding a new node:"+e.getMessage());
				throw new SemanticException("");
			}finally{
				this.disconnectBD(cb);
			}
		...
		public void setNode(int idNode, String sNType, Hashtable dto) throws SemanticException  {
			...
			for (int i =0;i<l.size();i++ ){
				...
				try {
					c=cb.getConnection();
					res =  AccesBD.executeUpdate("cur_"+sNType,"id_node = "+idNode,upd,c);					
					if (res <0 ){
						c.close();
		...
		public void delNode(int idNode, String sNType) throws SemanticException {
			...
			try {
				...
				res = AccesBD.executeDelete("cur_"+sNType, "id_node = "+idNode,c);
				c.close();
		...
		public void addNewRelation(Hashtable dto) throws SemanticException {
			...
			try {
				//Inserta el node a la BBDD.
				c = cb.getConnection();
				res = AccesBD.executeInsert(table,insert,c);
				c.close();
			}catch(Exception e){
				logger.error("Error on add new relation"+e.getMessage());
			}finally{
				this.disconnectBD(cb);
			}
		}
		...
		public void delRelation(Hashtable dto) throws SemanticException {
			...
			try{
				c = cb.getConnection();
				res = AccesBD.executeDelete((String) hConf.get("table"),condi,c);
				c.close();
			}catch(Exception e){
				logger.error("Error on delete a node:"+e.getMessage());
			}finally{
				this.disconnectBD(cb);
			}
		...
		public void delRelationsNode(int idNode, String sNType, String sRType) throws SemanticException {
			...
			if (sRType.compareTo("RLL")!=0 && sRType.compareTo("RAL")!=0 && sRType.compareTo("RN")!=0 && sRType.compareTo("RCC")!=0){
				try {
					//Inserta el node a la BBDD.
					c = cb.getConnection();
					res = AccesBD.executeDelete((String) hConf.get("table"),(String)hConf.get("condition"),c);
					c.close();
