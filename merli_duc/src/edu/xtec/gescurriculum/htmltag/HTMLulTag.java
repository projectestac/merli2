...
import edu.xtec.gescurriculum.gestorcurriculum.NodeForm;
	...
	/* (non-Javadoc)
	  ...
	  public String buildHTML() throws javax.servlet.jsp.JspException {
			  ...
			  */
			  ...
			  idNode = ((NodeForm)bean).getIdNode();		  
			  nodeType = String.valueOf(((NodeForm)bean).getNodeType());

			  if (this.id.compareTo("area") == 0){
				  result = String.valueOf(((NodeForm)bean).getIdArea());
				  try{
					  al = sn.getArees(Integer.parseInt(result),Integer.parseInt(String.valueOf(((NodeForm)bean).getIdLevel())));
					  al = Utility.orderListByPosition(al);
					  it = al.iterator();
					  result = createOLnew(it, Integer.parseInt(result));	
			  ...
			  if (this.id.compareTo("level") == 0){
				  result = String.valueOf(((NodeForm)bean).getIdLevel());
				  try{
					  al = sn.getLevels(Integer.parseInt(result));	
					  al = Utility.orderListByPosition(al);
					  it = al.iterator();
			  ...
			  if (this.id.compareTo("content") == 0){
				  value = String.valueOf(((NodeForm)bean).getIdContent());
				  if (value == null || Integer.parseInt(value)< 0){value = "0";}
				  try{
					  h = sn.getContents(Integer.parseInt(value),Integer.parseInt(String.valueOf(((NodeForm)bean).getIdLevel())),Integer.parseInt(String.valueOf(((NodeForm)bean).getIdArea())));
					  //Node n = sn.getNode(((NodeForm)bean).getIdContent(),"content");			 
				  }catch(Exception e){
				  }
					  al = (ArrayList) h.get("cp");
					  al = Utility.orderListByPosition(al);
					  it = al.iterator();
					  result = "<div id=\"contCP\" class=\"content\">";
					  result += createOLnew(it, Integer.parseInt(value));
					  result += "</div>";
					  if (((Integer) ((Hashtable)al.get(0)).get("idNode")).intValue() < 0){
						  result = result.replaceAll("<ol>","<ul>");
						  result = result.replaceAll("</ol>","</ul>");		
					  ...
					  al = Utility.orderListByPosition(al);
					  it = al.iterator();
					  aux = "<div id=\"contCC\" class=\"content\">";
					  aux += createOLnew(it, Integer.parseInt(value));
					  aux += "</div>";
					  if (((Integer) ((Hashtable)al.get(0)).get("idNode")).intValue() < 0){
						  aux = aux.replaceAll("<ol>","<ul>");
						  aux = aux.replaceAll("</ol>","</ul>");		
					  }
					  result += aux;
					  al = (ArrayList) h.get("ca");
					  al = Utility.orderListByPosition(al);
					  it = al.iterator();
					  aux = "<div id=\"contCA\" class=\"content\">";
					  aux += createOLnew(it, Integer.parseInt(value));
					  aux += "</div>";	
					  if (((Integer) ((Hashtable)al.get(0)).get("idNode")).intValue() < 0){
						  aux = aux.replaceAll("<ol>","<ul>");
			  ...
			  if (this.id.compareTo("objective") == 0){
				  value = String.valueOf(((NodeForm)bean).getIdObjective());
				  try{
					  h = sn.getObjectives(Integer.parseInt(String.valueOf(((NodeForm)bean).getIdLevel())),Integer.parseInt(String.valueOf(((NodeForm)bean).getIdArea())));				  								  
				  }catch(Exception e){} 
				  al = (ArrayList) h.get("op");
				  al = Utility.orderListByPosition(al);
				  it = al.iterator();
				  result = "<div id=\"objOP\" class=\"content\">";
				  result += createOLnew(it, Integer.parseInt(value));
				  result += "</div>";
				  if (((Integer) ((Hashtable)al.get(0)).get("idNode")).intValue() < 0){
					  result = result.replaceAll("<ol>","<ul>");
				  ...
				  al = (ArrayList) h.get("ot");
				  al = Utility.orderListByPosition(al);
				  it = al.iterator();
				  aux = "<div id=\"objOT\" class=\"content\">";
				  aux += createOLnew(it, Integer.parseInt(value));
				  aux += "</div>";
				  if (((Integer) ((Hashtable)al.get(0)).get("idNode")).intValue() < 0){
					  aux = aux.replaceAll("<ol>","<ul>");
			  ...
			  if (this.id.compareTo("thesaurus") == 0){
				  int valor;
				  value = String.valueOf(((NodeForm)bean).getIdKey());
				  if (value == null || Integer.parseInt(value) < 0){ valor = 0;
				  }else{ valor = Integer.parseInt(value);}
				  result = "<div id=\"path\" class=\"thesaurus\">";
				  result += "<a href=\"#\" id=\"closeThesaurus\" onclick=\"swapDisplay('formThesaure');\">X </a> <a href=\"#\" id=\"path0\" onclick=\"navigateTo(0);\">INICI/ </a>";
				  aux = ((NodeForm)bean).getNavPath();
				  String acID;
				  Node n;
				  ...
				  if (aux != null){
					  while (aux.lastIndexOf("#")>0 && !trobat){
						  ...
						  if (Integer.parseInt(acID)!=0){
							  n = sn.getNode(Integer.parseInt(acID),"thesaurus");
							  result+="/<a href=\"#\" onclick=\"navigateTo("+n.getIdNode()+");\">"+Utility.xmlEncode((String)n.getTerm())+"</a>";
						  }
					  }
				  }
				  result += "</div>";
				  result += "<div id=\"thesaurus\" class=\"thesaurus\">";
				  
				  result += createThesaurusList(((ArrayList)sn.getThesaurus(valor)), "add");
				
				  result += "</div>";
				  result += "<div id=\"keys\" class=\"thesaurus\">";
				  aux = ((NodeForm)bean).getSelecPath();			  
				  trobat = false;
				  if (aux != null){
					  while (aux.lastIndexOf("#")>0 && !trobat){
						  acID = aux.substring(0,aux.indexOf("#"));
						  aux = aux.substring(aux.indexOf("#")+1);
						  n = sn.getNode(Integer.parseInt(acID),"thesaurus");
						  result += "<a href=\"#\" onclick=\"delKey("+n.getIdNode()+");\" id=\"delkey"+n.getIdNode()+"\">-</a> ";
						  result+="<a href=\"#\" onclick=\"navigateTo("+n.getIdNode()+");\" id=\"key"+n.getIdNode()+"\">"+Utility.xmlEncode((String)n.getTerm())+"</a>   ";
					  }
				  }
		...
		while (it.hasNext()){
			...
			if (((Integer) h.get("idNode")).intValue() != 0) {
				...
				if(((Integer)h.get("idNode")).intValue() == value){
					if(((Integer)h.get("idNode")).intValue() == idNode && ((String)h.get("nodeType")).compareTo(nodeType)==0){
						temp += " class=\"selected\" ";
					}else{
