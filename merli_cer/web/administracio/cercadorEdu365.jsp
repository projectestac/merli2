...
<%
    ...
    Logger logger = Logger.getLogger("cercadorEdu365.jsp");
...
<jsp:include page="topAdministracio.jsp?selected=2" />	
<script language="Javascript">
    function doSubmit() {
        var myForm = document.modificar;
        for (i = 0; i < myForm.elements.length; i++) {
            if (myForm.elements[i].type == "text" && isNaN(myForm.elements[i].value)) {
                alert("<%=XMLCollection.getProperty("administracio.cercadorEdu365.errorNumeric")%>");
                myForm.elements[i].focus();
                return false;
            }
            if (myForm.elements[i].type == "text" && (myForm.elements[i].value < 0)) {
                alert("<%=XMLCollection.getProperty("administracio.cercadorEdu365.errorPositiu")%>");
                myForm.elements[i].focus();
                return false;
            }
            if (myForm.elements[i].type == "text" && (myForm.elements[i].value == '')) {
                alert("<%=XMLCollection.getProperty("administracio.cercadorEdu365.errorValor")%>");
                myForm.elements[i].focus();
                return false;
            }
...
<div id="content">
    <form name="modificar" action="/<%=Configuracio.contextWebAplicacio%>/ServletConfiguracio" method="post">     
        ...
        <fieldset>
            <legend><%=XMLCollection.getProperty("administracio.cercadorEdu365.parametres_cercador")%></legend>
            <p class="area_form"><%=XMLCollection.getProperty("administracio.cercadorEdu365.aparenca_resultats")%></p>     

            <div id="linia">
                <label class="cercador" for="longitudDescripcio"><%=XMLCollection.getProperty("administracio.cercadorEdu365.longitud_descripcio")%></label>
                <input type="text" name="longitudDescripcio" size="3" maxlength="3" value="<%=longitudDescripcio%>" />
            </div>
            <div id="linia">
                <label class="cercador" for="resultatsPagina"><%=XMLCollection.getProperty("administracio.cercadorEdu365.resultats_per_pagina")%></label>
                <input type="text" name="resultatsPagina" size="3" maxlength="3" value="<%=resultatsPagina%>"/>
            </div>
            <div id="linia">
                <label class="cercador" for="nombreNovetats"><%=XMLCollection.getProperty("administracio.cercadorEdu365.nombre_novetats")%></label>
                <input type="text" name="nombreNovetats" size="3" maxlength="3" value="<%=nombreNovetats%>"/>
            </div>
            <div id="linia">
                <label class="cercador" for="tempsVidaNovetat"><%=XMLCollection.getProperty("administracio.cercadorEdu365.temps_vida_novetat")%></label>
                <input type="text" name="tempsVidaNovetat" size="3" maxlength="3" value="<%=tempsVidaNovetat%>"/>
            </div>

            <p class="area_form"><%=XMLCollection.getProperty("administracio.cercadorEdu365.algoritme_ordenacio")%></p>

            <%
                stmt = conn.createStatement();
                sql = "select * from config_pesos_index where cercador_id=1";
                rs = stmt.executeQuery(sql);
                while (rs.next()) {
            ...
            <div id="linia">
                <input type="button" name="Modificar" value="<%=XMLCollection.getProperty("administracio.cercadorEdu365.modificar")%>" class="tableButton" onClick="javascript:doSubmit();"/>
