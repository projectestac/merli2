...
public class CopyFile {
	...
	// logger
	static final Logger logger = Logger.getLogger(simpple.xtec.web.util.CopyFile.class);
	...
	public static Long copyFile(File srcFile, File destFile)
		...
		if (clock) {
			...
			logger.info("Second(s): " + (millis/1000L));
		}
		if (verify) {
	...
	public static void main(String[] args) throws IOException {
		...
		if (verify) {
			...
			if (checksumSrc.equals(checksumDest)) {
				logger.debug(" OK, files are equal.");
			} else {
				logger.debug(" Error: Checksums differ.");
			}
		}
	}
	...
	public static boolean readYesNoFromStandardInput(String message) {
		...
		try
		{
			while ((line = in.readLine()) != null) {
				...
				{
					logger.debug("Could not understand answer (\"" +
